     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    UndistortKeyPoints();  // Get the mvKeysUn
     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
	...
	cout << "area:" << show.size().area() << "," << mvKeysUn.size() << endl;
	for (size_t i1 = 0, iend1 = mvKeysUn.size(); i1 < iend1; i1++) {
		cv::KeyPoint kp1 = mvKeysUn[i1];
		int level1 = kp1.octave;
		if (level1 > 0)
			continue;
		cv::circle(show, kp1.pt, 2, cv::Scalar(255), 2);
	}
	cv::imshow("test", show);
	//cv::waitKey();
	*/
